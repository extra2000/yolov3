...
from common.upsample import upsample
from common.backbone import darknet53
...
class YOLOv3:
    ...
    def __init__(self, input_data, trainable, config):
        """Implements TensorFlow YOLOv3

        Parameters
        ----------
        input_data : tensorflow.python.framework.ops.Tensor
        trainable : bool
        """
        ...
        self.anchors = np.array(config['yolov3']['anchors']['baseline'], dtype=np.float32).reshape(3, 3, 2)
        self.anchor_per_scale = config['yolov3']['anchor_per_scale']
        self.iou_loss_thresh = config['yolov3']['iou_loss_thresh']
        self.upsample_method = config['yolov3']['upsample_method']
